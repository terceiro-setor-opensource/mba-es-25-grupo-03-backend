...
namespace BaseAPI.Controllers
{
    ...
    public class CursoController : ApiController
    {
        ...
        public async Task<IActionResult> Get(string? descricao, string? categorias, int? ratingMin, int? ratingMax, int? duracaoMin, int? duracaoMax)
        {
            try
            {
                return Response(await _cursoBusiness.List(descricao, categorias, ratingMin ?? 0, ratingMax ?? 5, duracaoMin ?? 0, duracaoMax ?? 999), _cursoBusiness.Mensagem, false);
            }
            catch (Exception)
            {
